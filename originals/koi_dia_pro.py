...
# =============================================================================
# ETF CONFIGURATION (DIA, TLT, GLD, etc.)
# =============================================================================
# Set IS_ETF = True when testing ETFs
IS_ETF = True
...
# --- EXAMPLE DIA CONFIG (uncomment to test DIA) ---
DATA_FILENAME = 'DIA_5m_5Yea.csv'
FOREX_INSTRUMENT = 'DIA'
...
# Min SL Filter
# NOTE: For ETFs with PIP_VALUE=0.01, values are in "pips" where $1 = 100 pips
# DIA example: $3 SL = 300 pips, so use 100-2000 range for ETFs
USE_MIN_SL_FILTER = False  # Disable for DIA testing (or set 100-2000 for ETFs)
MIN_SL_PIPS = 10.5  # Forex: 10.5 pips | ETF: 100 ($1)

# Max SL Filter
USE_MAX_SL_FILTER = False  # Disable for DIA testing
MAX_SL_PIPS = 14.5  # Forex: 14.5 pips | ETF: 2000 ($20)

# ATR Filter
USE_ATR_FILTER = True
ATR_MIN_THRESHOLD = 0.20
ATR_MAX_THRESHOLD = 0.40

# =============================================================================
# COMMISSION CLASS - Forex (Darwinex Zero: $2.50/lot/order)
# =============================================================================
...
class KOIStrategy(bt.Strategy):
    params = dict(
        ...
        pip_value=PIP_VALUE,
        contract_size=100000 if not IS_ETF else ETF_CONTRACT_SIZE,
        is_etf=IS_ETF,
        margin_pct=MARGIN_PERCENT if not IS_ETF else ETF_MARGIN_PERCENT,
        lot_size=100000 if not IS_ETF else 1,
        print_signals=False,
    )
...
if __name__ == '__main__':
    ...
    # Data - Use appropriate data loader based on instrument type
    data_path = Path(__file__).parent.parent.parent / 'data' / DATA_FILENAME
    if not data_path.exists():
        data_path = Path(__file__).parent / DATA_FILENAME
    
    # All instruments use ForexCSVData for proper Date/Time column handling
    # GenericCSVData doesn't combine Date+Time columns correctly (shows 23:59:59)
    data = ForexCSVData(
    ...
    # Broker - Select commission based on instrument type
    cerebro.broker.setcash(STARTING_CASH)
    if IS_ETF:
        # ETF: $0.02/contract, 20% margin
        ...
        commission_info = f"${COMMISSION_PER_LOT_PER_ORDER}/lot (Darwinex Zero)"
