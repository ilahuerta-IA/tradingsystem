...
⚠️ OPTIMIZATION RESULT: MARGINAL (PF 1.46, close to 1.5 target)

OPTIMIZED CONFIG:
- SL Range: 10-18 pips (USE_MIN_SL_FILTER + USE_MAX_SL_FILTER)
- CCI Range: 100-130 (CCI_THRESHOLD + CCI_MAX_THRESHOLD)  
- Session: H03-17 (USE_SESSION_FILTER)
- Breakout: 5 pips offset, 5 candles window
- ATR SL/TP: 3.0x / 12.0x

RESULTS (5Y backtest 2020-2025):
- 147 trades, PF 1.46, WR 28.6%
- Sharpe 2.62, Max DD 6.06%
- Net P&L: +$21,905

YEARLY BREAKDOWN:
- 2020: 27 tr, PF 1.55, +$4,271 ✅
- 2021: 21 tr, PF 1.17, +$1,172 ✅
- 2022: 24 tr, PF 2.23, +$8,169 ✅
- 2023: 30 tr, PF 1.56, +$5,470 ✅
- 2024: 15 tr, PF 2.47, +$6,298 ✅
- 2025: 30 tr, PF 0.71, -$3,493 ❌

NOTE: 2025 underperforms - needs monitoring
...
# =============================================================================
# KOI PARAMETERS - TO BE OPTIMIZED
# =============================================================================

# EMAs (defaults - to optimize in Phase 3)
EMA_1_PERIOD = 10
EMA_2_PERIOD = 20
EMA_3_PERIOD = 40
EMA_4_PERIOD = 80
...
# CCI (CCI<130 = PF 1.69 from analysis)
CCI_PERIOD = 25
CCI_THRESHOLD = 100
...
# Breakout Window - relaxed for more trades
...
# =============================================================================
# FILTERS (defaults - to optimize in Phase 5)
# =============================================================================

# Session Filter - H03-17 (optimized from trade log analysis)
USE_SESSION_FILTER = True
ENTRY_START_HOUR = 3
ENTRY_END_HOUR = 17

# Min SL Filter - SL >= 10 pips (from trade log analysis)
USE_MIN_SL_FILTER = True
MIN_SL_PIPS = 10.0

# Max SL Filter - SL <= 18 pips (from trade log analysis)
...
class KOIStrategy(bt.Strategy):
    params = dict(
        ...
        cci_max_threshold=CCI_MAX_THRESHOLD,
    ...
    def _init_trade_reporting(self):
        if EXPORT_TRADE_REPORTS:
            try:
                ...
                report_path = report_dir / f"KOI_{FOREX_INSTRUMENT}_trades_{timestamp}.txt"
                self.trade_report_file = open(report_path, 'w', encoding='utf-8')
                self.trade_report_file.write(f"=== KOI STRATEGY TRADE REPORT ({FOREX_INSTRUMENT}) ===\n")
    ...
    def _check_cci_condition(self):
        try:
            ...
            # CCI must be above threshold but NOT too extreme (>200 = all losses)
            return cci_val > self.p.cci_threshold and cci_val < self.p.cci_max_threshold
        except:
            return False
    ...
    def stop(self):
        ...
        # Print summary
        print("\n" + "=" * 70)
        print(f"=== KOI STRATEGY SUMMARY ({FOREX_INSTRUMENT}) ===")
        print("=" * 70)
